...
def faster_rating_prediction(k, user_similarity_matrix, time, alpha, user_timestamp_array, user_rating_array, num_user, num_movie):
    ...
    print("user_rating_array: ", user_rating_array)
    ...
    for target_user_index in range(0, num_user):
        ...
        print("avg_rating_of_target_user: ", avg_rating_of_target_user)
...
def temporal_CF_decay_function_binary(data_set, num_user, num_movie, time_window, alpha=1.7, k=11):
    ...
    for user_id in range(1,num_user+1):
        ...
        user_rating_dict[user_id]=np.array([-1]*num_movie)

    user_timestamp_dict={} # size is num_user
    for user_id in range(1,num_user+1):
        user_timestamp_dict[user_id]=np.array([-1]*num_movie)
    ...
    for user_id, each in user_rating_dict.items():
        print("user_id: ", user_id, " each: ", each)
        if user_id in user_id_list:
            user_rating_array.append(each) # valid rating is 1-5
        else:
            user_rating_array.append([-1]*num_movie)
    ...
    for i in range(0,num_user):
        ...
        #     print(i)
        row_list=[]
        for j in range(0,num_user):  # NOTE: we can't do this since -1 * -1 = 1 ...
