...
for i in range(len(window_size_unit_list)):
    ...
    first_label.set_transform(
        first_label.get_transform() + mtransforms.Affine2D().translate(-13, 3))  # Shift 5 points left
    first_label = axs[axis_number[i]].get_xticklabels()[1]
    first_label.set_transform(
        first_label.get_transform() + mtransforms.Affine2D().translate(10, 0))  # Shift 5 points left
...
axs.flat[5].set_yticks([0.5, 0.6, 0.7, 0.8, 0.9], ['0.5', '0.6', '0.7', '0.8', '0.9'], fontsize=15)
...
x = fig.legend(title_fontsize=14, loc='upper left', bbox_to_anchor=(0, 1.25), fontsize=15,
...
plt.savefig(f"movielens_accuracy_fixed_windows_change_window_size_alpha_{str(get_integer(alpha))}.png", bbox_inches='tight')
plt.show()
