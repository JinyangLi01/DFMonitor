...
if len(sys.argv) > 1:
    window_size_units = sys.argv[1]
else:
    raise ValueError("No window size provided.")
...
DFMonitor_bit, DFMonitor_counter, uf_list, accuracy_list, counter_list_correct, counter_list_incorrect, \
    ...
    = workload.traverse_data_DFMonitor_and_basel.ine(data, date_column,
                                                    date_time_format,
                                                    monitored_groups,
...
counter_file_name = f"movielens_compare_Counters_{method_name}_gender_per_item_alpha_{str(get_integer(alpha))}_time_unit_{time_unit}*{window_size_units}_check_interval_{checking_interval}.csv"
with open(counter_file_name, "w", newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerow(["counter_list_correct_Male", "counter_list_correct_Female",
                     "counter_list_incorrect_Male", "counter_list_incorrect_Female"])
    for i in range(len(counter_list_correct)):
        writer.writerow([counter_list_correct[i][0], counter_list_correct[i][1],
                         counter_list_incorrect[i][0], counter_list_incorrect[i][1]])
