...
checking_interval_units = "1 hour"

(DFMonitor_bit, DFMonitor_counter, uf_list, accuracy_list, counter_list_correct, counter_list_incorrect,
 window_reset_times, check_points) = workload.traverse_data_DFMonitor_and_baseline(data, date_column,
                                                date_time_format, monitored_groups,
                                                threshold, alpha, time_unit, window_size_units,
                                                checking_interval_units, label_prediction,
...
filename = f"movielens_compare_Accuracy_{method_name}_gender_per_item_alpha_{str(get_integer(alpha))}_time_unit_{time_unit}*{window_size_units}.csv"
