...
window_size_unit_list = ['1', '24*30*2']
...
axs.flat[0].set_yticks([0.5, 0.7, 0.9, 1.0], [0.5, 0.7, 0.9, 1.0], fontsize=14)
axs.flat[0].set_xlabel('(a) 1 second', fontsize=13, labelpad=3, fontweight='bold').set_position([0.4, -0.1])
axs.flat[0].set_ylim(0.5, 1)

axs.flat[1].set_yticks([0.5, 0.7, 0.9, 1.0], [0.5, 0.7, 0.9, 1.0], fontsize=14)
axs.flat[1].set_xlabel('(b) 2 months', fontsize=13, labelpad=3, fontweight='bold').set_position([0.4, -0.1])
...
plt.subplots_adjust(left=0, right=0.98, top=0.8, bottom=0.05, wspace=0, hspace=0)
#

# Create a common legend using fig.legend()
fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.1), ncol=2, title_fontsize=14,
           labelspacing=0.5, handletextpad=0.4, markerscale=2.5, handlelength=1.5,
           columnspacing=0.5, borderpad=0.2, frameon=True)


plt.tight_layout()
plt.savefig("movielens_accuracy_fixed_windows_change_sizes_3.png", bbox_inches='tight')
plt.show()
