...
alpha_list = [0.5, 0.9, 0.97, 0.993, 0.995, 0.999, 0.9995]
window_size = "24"
result_file_list = dict()
for alpha in alpha_list:
    file_name = f"movielens_compare_Accuracy_hoeffding_classifier_gender_per_item_alpha_{get_integer(alpha)}_time_unit_1 hour*{window_size}_check_interval_1 hour.csv"
    # if file exists
    if not os.path.exists(file_name):
        subprocess.run(["python3", "accuracy_gender_per_item.py", str(alpha)])
        # Check again after running the script
        if not os.path.exists(file_name):
            raise Exception(f"Error: {file_name} was not created by the script.")
    df = pd.read_csv(file_name)
    print("alpha", alpha, df)
    result_file_list[alpha] = df
...
fig, axs = plt.subplots(1, 7, figsize=(11, 0.85))
plt.subplots_adjust(top=0.7, bottom=0.0, hspace=0, wspace=0.33, left=-0.1, right=1)
...
df = result_file_list[alpha_list[4]]
...
for i in range(len(alpha_list)):
    alpha = alpha_list[i]
    df = result_file_list[alpha]
    x_list = np.arange(0, len(df))
    male_time_decay = df["male_time_decay"].tolist()
    female_time_decay = df["female_time_decay"].tolist()
    axs[axis_number[i]].plot(x_list, male_time_decay, linewidth=1, markersize=1, label='Male', linestyle='-', marker='o', color="blue")
    axs[axis_number[i]].plot(x_list, female_time_decay, linewidth=1, markersize=1, label='Female', linestyle='-', marker='o',
            color="darkorange")
    xlabel = f"({chr(97 + i)}) $\\alpha$={alpha}"
    axs[axis_number[i]].set_xlabel(xlabel, fontsize=15, labelpad=1).set_position([0.4, -0.3])
    axs.flat[axis_number[i]].set_yticks([0.6, 0.7, 0.8, 0.9], ['0.6', '0.7', '0.8', '0.9'], fontsize=15)
    axs.flat[i].set_xticks([], [], rotation=0, fontsize=15)
    ...
    axs[axis_number[i]].set_xticks(xticks_idx, xtick_label, rotation=10, fontsize=15)
    axs[axis_number[i]].tick_params(axis='x', which='major', pad=-4)
    ...
    first_label.set_transform(
        first_label.get_transform() + mtransforms.Affine2D().translate(-13, -1))  # Shift 5 points left
    first_label = axs[axis_number[i]].get_xticklabels()[1]
    first_label.set_transform(
        first_label.get_transform() + mtransforms.Affine2D().translate(10, -3))  # Shift 5 points left
    axs[axis_number[i]].grid(True)

axs.flat[5].set_ylim(0.7, 0.9)
axs.flat[5].set_yticks([0.7, 0.75, 0.8, 0.85, 0.9], ['0.7', '', '0.8', '', '0.9'], fontsize=15)
axs.flat[6].set_ylim(0.7, 0.9)
axs.flat[6].set_yticks([0.7, 0.75, 0.8, 0.85, 0.9], ['0.7', '', '0.8', '', '0.9'], fontsize=15)
...
x = fig.legend(loc='upper left', bbox_to_anchor=(-0.1, 1.1), fontsize=15,
           ncol=2, labelspacing=1, handletextpad=0.4, markerscale=8,
...
plt.savefig("movielens_accuracy_fixed_windows_change_alpha.png", bbox_inches='tight')
plt.show()
