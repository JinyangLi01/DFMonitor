...
plt.subplots_adjust(top=0.82, bottom=0.32, hspace=0, wspace=0.28, left=0.0, right=0.99)
...
df = result_file_list[alpha_list[1]]
...
for i in range(len(alpha_list)):
    ...
    axs[axis_number[i]].set_xlabel(xlabel, fontsize=14, labelpad=1).set_position([0.4, -0.3])
    axs.flat[axis_number[i]].set_yticks([0.6, 0.7, 0.8, 0.9], ['0.6', '0.7', '0.8', '0.9'], fontsize=14)
    ...
    first_label.set_transform(
        first_label.get_transform() + mtransforms.Affine2D().translate(-13, 2))  # Shift 5 points left
    first_label = axs[axis_number[i]].get_xticklabels()[1]
    first_label.set_transform(
        first_label.get_transform() + mtransforms.Affine2D().translate(10, 2))  # Shift 5 points left
...
x = fig.legend(loc='upper left', bbox_to_anchor=(0, 1.1), fontsize=14,
