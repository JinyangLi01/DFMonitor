...
plt.subplots_adjust(top=0.85, bottom=0.32, hspace=0, wspace=0.25, left=0.06, right=0.99)
...
for i in range(len(Tb_list)):
    Tb = Tb_list[i]
    df = result_file_list[Tb]
    x_list = np.arange(0, len(df))
    df["check_points"] = pd.to_datetime(df["check_points"])
    check_points = df["check_points"].tolist()
    male_time_decay = df["male_time_decay"].tolist()
    female_time_decay = df["female_time_decay"].tolist()
    axs[axis_number[i]].plot(x_list, male_time_decay, linewidth=1, markersize=1.0, label='Male', linestyle='-', marker='o', color="blue")
    axs[axis_number[i]].plot(x_list, female_time_decay, linewidth=1, markersize=1.0, label='Female', linestyle='-', marker='o',
            color="orange")
    ...
    axs[axis_number[i]].tick_params(axis='y', which='major', pad=3)
    first_label = axs[axis_number[i]].get_xticklabels()[0]
    first_label.set_transform(first_label.get_transform() + mtransforms.Affine2D().translate(-13, 1))  # Shift 5 points left
    first_label = axs[axis_number[i]].get_xticklabels()[1]
    first_label.set_transform(first_label.get_transform() + mtransforms.Affine2D().translate(10, 1))  # Shift 5 points left
...
# labels[0].set_y(0.16)
# labels[0].set_x(labels[0].get_position()[0] - 30)
# labels[1].set_y(0.12)
...
#
# Tb_list = [24, 24*3, 24*7, 24*7*4, 24*7*4*2, 24*7*4*3]

# axs[0, 0].set_yscale('log')
axs.flat[0].set_yticks([0.6, 0.7, 0.8, 0.9], ['0.6', '0.7', '0.8', '0.9'], fontsize=14)
axs.flat[0].set_xlabel('(a) $T_b =$ 1 day', fontsize=14, labelpad=-2).set_position([0.4, 1])
axs.flat[1].set_yticks([0.6, 0.7, 0.8, 0.9], ['0.6', '0.7', '0.8', '0.9'], fontsize=14)
axs.flat[1].set_xlabel('(b) $T_b =$ 3 days', fontsize=14, labelpad=-2).set_position([0.4, -0.1])
# axs.flat[2].set_yscale('symlog', linthresh=0.22)
axs.flat[2].set_yticks([0.6, 0.7, 0.8, 0.9], ['0.6', '0.7', '0.8', '0.9'], fontsize=14)
axs.flat[2].set_xlabel('(c) $T_b =$ 1 week', fontsize=14, labelpad=-2).set_position([0.4, -0.1])
...
# plt.tight_layout()
plt.savefig("movielens_accuracy_adaptive_windows_change_Tb.png")
plt.show()
