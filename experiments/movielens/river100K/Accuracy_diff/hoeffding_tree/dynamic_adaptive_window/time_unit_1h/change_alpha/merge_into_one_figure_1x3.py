...
for alpha in alpha_list:
    file_name = f"movielens_compare_Accuracy_hoeffding_classifier_gender_per_item_alpha_{get_integer(alpha)}_time_unit_1 hour_check_interval_{checking_interval}_Tin_{Tin}_Tb_{Tb}.csv"
    # if file exists
    if not os.path.exists(file_name):
        subprocess.run(["python3", "accuracy_gender_per_item_1hour.py", str(alpha)])
...
plt.subplots_adjust(top=0.84, bottom=0.3, hspace=0, wspace=0.25, left=0.05, right=0.99)
...
curve_colors = sns.color_palette(palette=[ '#0000ff', 'cyan', '#57d357', '#004b00', 'darkorange', 'firebrick',
                                           'blueviolet', 'magenta'])
curve_colors = sns.color_palette(['blue', 'orange'])
...
df = result_file_list[alpha_list[0]]
...
for i in range(len(alpha_list)):
    ...
    axs[axis_number[i]].plot(x_list, female_time_decay, linewidth=1, markersize=1.2, label='Female', linestyle='-', marker='o',
            color="orange")
    ...
    axs[axis_number[i]].tick_params(axis='x', which='major', pad=0)
    axs[axis_number[i]].tick_params(axis='y', which='major', pad=0)
    first_label = axs[axis_number[i]].get_xticklabels()[0]
    first_label.set_transform(first_label.get_transform() + mtransforms.Affine2D().translate(-13, -2))  # Shift 5 points left
    first_label = axs[axis_number[i]].get_xticklabels()[1]
    first_label.set_transform(first_label.get_transform() + mtransforms.Affine2D().translate(10, -2))  # Shift 5 points left
    axs[axis_number[i]].grid(True)
    if i >= 1:
        axs.flat[i].set_ylim(0.65, 0.9)
        axs[axis_number[i]].set_yticks([0.7, 0.8, 0.9], ['0.7', '0.8', '0.9'], fontsize=14)
    else:
        axs.flat[i].set_ylim(0.5, 1.0)
        axs[axis_number[i]].set_yticks([0.6, 0.8, 1.0], ['0.6', '0.8', '1.0'], fontsize=14)
    xlabel = f"({chr(97 + i)}) $\\alpha = {alpha}$"
    axs[axis_number[i]].set_xlabel(xlabel, fontsize=14, labelpad=-1).set_position([0.4, -0.1])
...
x = fig.legend(title_fontsize=14, loc='upper left', bbox_to_anchor=(0, 1.08), fontsize=14,
           ncol=2, labelspacing=1, handletextpad=0.4, markerscale=5,
...
plt.savefig("movielens_accuracy_adaptive_windows_change_alpha.png")
plt.show()
