...
df = pd.read_csv(f"movielens_compare_Accuracy_{method_name}_gender_dynamic_Tb_{T_b/eval(window_size_units)}_Tin_{T_in/eval(window_size_units)}_alpha_{str(get_integer(alpha))}_time_unit_{time_unit}_check_interval_{checking_interval}.csv")
print(df)
