...
def plot_certain_time_window(window_size, axs):
    ...
    print("len ", len(df_male), len(df_female))
    ...
    axs.legend(loc='lower right',
           bbox_to_anchor=(1.0, 0.08),  # Adjust this value (lower the second number)
    ...
    axs.set_xlabel('timestamps, window size = 1 day', fontsize=16)
    ...
    for i in range(0, len(df_male)):
        if (female_lst[i - 1] - female_lst[i]) > decline_threshold:
            plt.axvline(x=i, color='black', linestyle=(0, (5, 5)), linewidth=1.5, alpha=0.9)
    ...
    axs.annotate('', xy=(x[5], y[5]), xytext=(x[5], y[6]),
                  arrowprops=dict(arrowstyle='-', linestyle=':', color='black', lw=1.5),
                 fontsize=12, ha='center')
    axs.annotate('', xy=(2, y[6]), xytext=(7.3, y[6]),
                  arrowprops=dict(arrowstyle='-', linestyle=":", color='black', lw=1.5),
                 fontsize=12, ha='center')
    plt.text(2, 0.62, round(y[5] - y[6], 2), fontsize=11, va='bottom')

    # Add annotation with the difference
    axs.annotate('', xy=(x[17], y[17]), xytext=(x[17], y[18]),
                 arrowprops=dict(arrowstyle='-', linestyle=':', color='black', lw=1.5),
                 fontsize=12, ha='center')
    axs.annotate('', xy=(15, y[18]), xytext=(19, y[18]),
                 arrowprops=dict(arrowstyle='-', linestyle=":", color='black', lw=1.5),
                 fontsize=12, ha='center')
    plt.text(14, 0.7, round(y[17] - y[18], 2), fontsize=11, va='bottom')
    ...
    # plt.axvline(x=23, color='black', linestyle=(0, (5, 5)), linewidth=1.5, alpha=0.9)
    plt.axvline(x=26, color='black', linestyle=(0, (5, 5)), linewidth=1.5, alpha=0.9)

    #
    # plt.xticks(decline_points+[26], [datetime[decline_points[i]].replace(" ", "\n") for i in range(len(decline_points))] + [datetime[26].replace(" ", "\n")],
    #            color='black',
