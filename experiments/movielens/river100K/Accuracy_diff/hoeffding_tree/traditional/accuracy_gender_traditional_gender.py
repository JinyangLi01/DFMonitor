...
# Convert datetime column to only include date part
data['datetime'] = pd.to_datetime(data['datetime']).dt.normalize()


# Define a function to calculate accuracy
def calculate_accuracy(group):
    correct = len(group[group['diff_binary_correctness'] == 1])
    total = len(group)
...
for window_size in window_size_list:
    # Perform the grouping, keeping partial periods with "dropna=False"
    grouped = data.groupby(["gender", pd.Grouper(key="datetime", freq=window_size, dropna=False)])

    # Iterate over each group and calculate accuracy separately
    results = []
    for (gender, period), group in grouped:
        calculated_value = calculate_accuracy(group)
        results.append({'gender': gender, 'datetime': period, 'calculated_value': calculated_value})

    # Manually construct a DataFrame, retaining original columns
    acc_df = pd.DataFrame(results)

    # Fill in the last partial period if missing for both female and male for each time window
    for gender in ['F', 'M']:
        gender_data = data[data['gender'] == gender]
        gender_acc_df = acc_df[acc_df['gender'] == gender]
        last_date = gender_data['datetime'].max()
        last_window_start = gender_acc_df['datetime'].max()

        if last_window_start < last_date:
            partial_period = gender_data[
                (gender_data['datetime'] >= last_window_start) & (gender_data['datetime'] <= last_date)]
            if not partial_period.empty:
                calculated_value = calculate_accuracy(partial_period)
                acc_df = pd.concat([acc_df, pd.DataFrame(
                    {'gender': [gender], 'datetime': [last_window_start], 'calculated_value': [calculated_value]})],
                                   ignore_index=True)

    acc_df.sort_values(by=['gender', 'datetime'], inplace=True)
    acc_df.reset_index(drop=True, inplace=True)
    acc_df.to_csv(f"movielens_compare_Accuracy_{method_name}_traditional_gender_{window_size}.csv")
    print(acc_df)
