...
def plot_certain_time_window(window_size, axs):
    ...
    axs.legend(loc='lower left',
           bbox_to_anchor=(0, 0),  # Adjust this value (lower the second number)
    ...
    axs.set_xlabel('timestamps, window size = 2 weeks', fontsize=16)
    ...
    axs.annotate('', xy=(7, y[9]), xytext=(10, y[9]),
                  arrowprops=dict(arrowstyle='-', linestyle=":", color='black', lw=1.5),
                 fontsize=12, ha='center')
    plt.text(6.1, 0.71, round(y[8] - y[9], 2), fontsize=14, va='bottom')
    ...
    axs.annotate('', xy=(11, y[13]), xytext=(14, y[13]),
                 arrowprops=dict(arrowstyle='-', linestyle=":", color='black', lw=1.5),
                 fontsize=12, ha='center')
    plt.text(10.4, 0.71, round(y[12] - y[13], 2), fontsize=14, va='bottom')
    #

    plt.ylabel('accuracy', fontsize=15)
    axs.grid(axis='x')
    plt.yticks([ 0.7, 0.8], fontsize=15)
    plt.xticks(decline_points, [" " + datetime[decline_points[i]] for i in range(len(decline_points))], color='black',
                rotation=0, fontsize=12.2)
