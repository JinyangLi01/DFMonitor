...
window_size_str_list = ["2 week", "1 month", "2 month", "4 month", "6 month", "8 month",
                        "12 month", "24 month"]
window_size_str_list_brev = ["2w", "1m", "2m", "4m", "6m", "8m", "12m", "24m"]
threshold = 0.3
alpha = 0.5
...
black_time_decay = []
for time_window_str in window_size_str_list:
    ...
    (DFMonitor, uf_list_DF, fpr_list_DF, counter_list_TN_DF,
     counter_list_FP_DF) = workload.traverse_data_DF_baseline(data, date_column,
                                                              time_window_str,
                                                              monitored_groups,
                                                              threshold,
                                                              alpha)
...
with open("FPR_window_size.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerow(["window_size", "black_time_decay", "white_time_decay",
                     "hispanic_time_decay"])
    for i in range(len(window_size_str_list)):
        writer.writerow([window_size_str_list[i], black_time_decay_dif_alpha[i],
...
# plt.yticks([0.3, 0.4, 0.5, 0.6], ['0.3', '0.4', '0.5', '0.6'])
#
...
# plt.legend(title='Alpha Values', title_fontsize=17, loc='lower left', bbox_to_anchor=(-0.08, 0.96), fontsize=20,
#            ncol=4, labelspacing=0.1, handletextpad=0.2, markerscale=1.4,
#            columnspacing=0.2, borderpad=0.2, frameon=True, handles=handles, labels=labels)
#
# plt.savefig("FPR_window_size_black.png", bbox_inches='tight')
# plt.show()
...
# plt.yscale('log')
# plt.yscale('symlog', linthresh=0.22)
# plt.yticks([0.15, 0.2, 0.3, 0.4], ['0.15', '0.2', '0.3', '0.4'])
# plt.xlabel('normalized measuring time',
#            fontsize=20, labelpad=-2).set_position((0.47, -0.1))
# plt.ylabel('false positive rate (FPR)', fontsize=20, labelpad=-1)
# plt.grid(True)
...
#            columnspacing=0.2, borderpad=0.2, frameon=True, handles=handles, labels=labels)
#
# plt.savefig("FPR_window_size_white.png", bbox_inches='tight')
# plt.show()
#
# ############################################## hispanic ####################################################
# #
...
# # plt.yscale('log')
# plt.yscale('symlog', linthresh=0.35)
...
#            columnspacing=0.2, borderpad=0.2, frameon=True, handles=handles, labels=labels)
#
# plt.savefig("FPR_window_size_hispanic.png", bbox_inches='tight')
# plt.show()
