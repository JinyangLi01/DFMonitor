...
df = pd.read_csv(f"stocks_compare_Accuracy_{method_name}_sector_per_item_{exmained_time}_alpha_{str(get_integer(alpha))}_time_unit_{time_unit}*{window_size_units}_check_interval_{checking_interval}.csv")
df["check_points"] = pd.to_datetime(df["check_points"])
print(len(df))
df = df[df["check_points"] >= datetime(2024, 9, 18, 9, 10, 0)]
...
#                '#287c37', '#cccc00']
pair_colors = ["blue", "darkorange", "green"]
#
