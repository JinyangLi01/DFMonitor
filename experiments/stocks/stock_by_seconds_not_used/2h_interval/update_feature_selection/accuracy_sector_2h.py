...
chunk_size = 200
data = pd.read_csv('prediction_result_s&p50_all_stocks_2024-09-18_sorted_with_binary_direction_' + exmained_time + "_chunk_size_" + str(chunk_size) + '.csv')
...
monitored_groups = [{"sector": 'Technology'}, {"sector": 'Consumer Cyclical'}, {'sector': 'Communication Services'}, {'sector': 'Financial Services'}, {'sector': 'Consumer Defensive'}, {'sector': 'Healthcare'}]
print(data[:5])
alpha = 0.997
...
filename = f"stocks_compare_Accuracy_{method_name}_sector_per_item_{exmained_time}_alpha_{str(get_integer(alpha))}_time_unit_{time_unit}*{window_size_units}_check_interval_{checking_interval}_chunk_size_{chunk_size}.csv"

with open(filename, "w", newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerow(["Technology_time_decay", "ConsumerCyclical_time_decay", "CommunicationServices_time_decay", "FinancialServices_time_decay", "ConsumerDefensive_time_decay", "Healthcare_time_decay", "check_points"])
    for i in range(len(Technology_time_decay)):
        writer.writerow([accuracy_list[i][0], accuracy_list[i][1], accuracy_list[i][2],  accuracy_list[i][3],  accuracy_list[i][4],  accuracy_list[i][5], check_points[i]])
