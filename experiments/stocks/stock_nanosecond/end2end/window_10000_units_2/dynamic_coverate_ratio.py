...
result_file_name = f"dynamic_CR_alpha_{str(get_integer(alpha))}.csv"
...
time_end = pd.Timestamp('2024-10-15 14:01:00.00', tz='UTC')
...
print("len of selected data", len(data_stream))
...
for idx, row in data_stream.iterrows():
    ...
    if time_delta_check > checking_interval_nanoseconds:
        while (previous_check_timestamp + pd.Timedelta(
            ...
            all_cur_cr = [x / counters_all for x in counters_groups]
