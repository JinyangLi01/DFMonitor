...
removed_percentage = [50, 60, 70, 80, 90]
fig, ax = plt.subplots(figsize=(3.5, 1.8))
plt.subplots_adjust(left=0.1, right=0.9, top=0.8, bottom=0.1)

for stock_fraction in range(5, 10):
    print("stock fraction", stock_fraction, "color", pair_colors[stock_fraction-5])
    ...
    ax.plot(x_list, df[stock_sector].tolist(), linewidth=1, markersize=1, label=f"Remove {stock_fraction}%", linestyle='-',
            marker='o', color=pair_colors[stock_fraction-5], alpha=0.5)
