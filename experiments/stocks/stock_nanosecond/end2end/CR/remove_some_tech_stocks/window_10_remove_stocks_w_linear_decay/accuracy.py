...
data_file_name = f"prediction_result_end2end_window_10_decay_rate_{get_integer(alpha)}_remove_stocks_w_linear_decay.csv"
...
filename = (f"stocks_compare_Accuracy_decay_rate_{str(get_integer(alpha))}"
            f"_window_{window_size_units}_remove_stocks_w_linear_decay.csv")
...
plt.xticks([], [])
plt.yticks([0.4, 0.5, 0.6], fontsize=13)
plt.ylim(0.28, 0.6)
...
plt.savefig(f"StockAcc_end2end_alpha_{get_integer(alpha)}_remove_w_linear_decay.png",
            bbox_inches='tight')
plt.show()
