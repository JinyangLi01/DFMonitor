...
# Create the legend
leg = ax[-1].legend(loc="upper left", fontsize=16,
...
plt.savefig(f"Stock_acc_time_decay_sector_{date}_{time_period}_alpha_{str(get_integer(alpha))}"
            f"_time_unit_{time_unit}*_check_interval_{checking_interval}_Tin_{Tin}_Tb_{Tb}_"
            f"start_{time_start}_end_{time_end}_fig_{time_start_picture}_to_{time_end_picture}.png",
            bbox_inches='tight')
plt.show()
