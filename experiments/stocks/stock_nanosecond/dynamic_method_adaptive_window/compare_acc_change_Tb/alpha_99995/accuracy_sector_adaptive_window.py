...
for i in range(len(curve_names)):
    ax.plot(x_list, df[curve_names[i]+"_time_decay"].tolist(), linewidth=1, markersize=1,
...
plt.savefig(f"Stock_acc_{date}_{time_period}_a{str(get_integer(alpha))}_tu{time_unit}_ci{checking_interval}_"
            f"Tin_{Tin}_Tb_{Tb}_start_{time_start}_end_{time_end}_fig_{draw_figure_start_time}_to_{draw_figure_end_time}.png",
            bbox_inches='tight')
