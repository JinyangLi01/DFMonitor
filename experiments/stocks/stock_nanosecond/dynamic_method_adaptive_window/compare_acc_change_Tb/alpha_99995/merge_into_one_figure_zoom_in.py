...
Tb_list = [1000, 2000, 5000, 6000, 8000, 100000]
Tin = 10000
...
fig, axes = plt.subplots(1, 6, figsize=(14.6, 1.7), constrained_layout=True)
...
for a, Tb in enumerate(Tb_list):
    filename = (f"stocks_compare_Accuracy_sector_{date}_{time_period}_alpha_{str(get_integer(alpha))}"
                f"_time_unit_{time_unit}_check_interval_{checking_interval}_Tin_{Tin}_Tb_{Tb}.csv")
...
handles, labels = [], []
h, l = axes.flat[-2].get_legend_handles_labels()
handles.extend(h)
labels.extend(l)
...
fig.legend(handles, labels, loc="upper left", fontsize=16,
                bbox_to_anchor=(0.02, 0.5), ncol=7,
