...
Tb_list = [500, 1000, 10000, 20000, 30000, 50000]
Tin = 500
...
plt.subplots_adjust(top=0.84, bottom=0.32, hspace=0, wspace=0.3, left=0.03, right=0.99)
...
for a, Tb in enumerate(Tb_list):
    ...
    axes[a].set_yticks([0.3, 0.4, 0.6, 0.8], [0.3, 0.4, 0.6, 0.8], fontsize=15)
...
leg = axes[-1].legend(loc="upper left", fontsize=16,
                ...
                handletextpad=0.5, markerscale=16, handlelength=1.2, handleheight=1,
...
plt.tight_layout()
fig.savefig(f"compare_accuracy_time_decay_adaptive_window_change_Tb_zoom_in_{time_start_picture}---{time_end_picture}.png")
plt.show()
