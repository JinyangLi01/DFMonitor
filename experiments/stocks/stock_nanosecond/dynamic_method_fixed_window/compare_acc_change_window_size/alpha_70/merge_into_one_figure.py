...
window_size_units_list = [0.1,  1, 10]
...
fig, axes = plt.subplots(1, 5, figsize=(14.6, 1.7), constrained_layout=True)
...
for a, window_size in enumerate(window_size_units_list):
    ...
    time_end = pd.Timestamp('2024-10-15 14:00:10.30', tz='UTC')

    # Filter the DataFrame
    df = df[df["check_points"] >= time_start]
    df = df[df["check_points"] <= time_end]
    ...
    xlabel = f"({chr(97 + a)}) window size = {window_size}"
...
plt.tight_layout()
fig.savefig("compare_accuracy_time_decay_fixed_window_change_window_size.png")
plt.show()
