...
filename = (f"stocks_compare_Accuracy_sector_{date}_{time_period}_alpha_{str(get_integer(alpha))}"
            f"_time_unit_{time_unit}*{window_size_units}_check_interval_{checking_interval}_v8.csv")
...
plt.xticks([], [])
plt.yticks([0.5, 0.55, 0.6, 0.65, 0.7], fontsize=13)
...
plt.savefig(f"Stock_acc_time_decay_sector_{date}_{time_period}_alpha_{str(get_integer(alpha))}"
            f"_time_unit_{time_unit}*{window_size_units}_check_interval_{checking_interval}_v8.png", bbox_inches='tight')
plt.show()