...
filename = (f"stocks_compare_Accuracy_sector_{date}_{time_period}_alpha_{str(get_integer(alpha))}"
            f"_time_unit_{time_unit}*{window_size_units}_check_interval_{checking_interval}_v6.csv")
...
for i in range(len(curve_names)):
    ax.plot(x_list, df[curve_names[i]+"_time_decay"].tolist(), linewidth=0.7, markersize=1, label=curve_names[i], linestyle='-',
...
plt.tight_layout()
plt.legend(loc='upper left', bbox_to_anchor=(-0.25, 1.6), fontsize=11,
...
plt.savefig(f"Stock_acc_time_decay_sector_{date}_{time_period}_alpha_{str(get_integer(alpha))}"
            f"_time_unit_{time_unit}*{window_size_units}_check_interval_{checking_interval}_v6.png", bbox_inches='tight')
plt.show()